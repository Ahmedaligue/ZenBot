...
export default {
  ...
  help: ['𝚆𝙸𝙺𝙸𝙿𝙴𝙳𝙸𝙰 <𝚝𝚎𝚛𝚖𝚒𝚗𝚘>'],
  tags: ['🔍 𝗕𝗨́𝗦𝗤𝗨𝗘𝗗𝗔𝗦'],
  run: async (m, { text, conn, prefix, command, chatId }) => {
    if (!text) return m.reply(`🔎 *Dale, mandame qué querés buscar en Wikipedia, capo.*\nEjemplo: ${prefix + command} Messi`);
    ...
    if (!result.found) {
      return m.reply('❌ *No encontré nada copado en Wikipedia ni en español ni en inglés.*');
    }
